...
n_traj = st.sidebar.slider("Tracked digits", 3, 15, 8)
...
speed = st.sidebar.slider("Animation speed (ms/frame)", 50, 400, 150)

# --- Prepare trajectories ---
indices = (labels_2d == label).nonzero(as_tuple=True)[0][:n_traj]
orig_points = torch.tensor(images_2d[indices], dtype=torch.float32)
traj = [orig_points.numpy()]
for t in range(1, steps):
    noisy = q_sample(orig_points, t)
    traj.append(noisy.numpy())
...
def make_frame(frame):
    ...
    for i in range(n_traj):
        ...
        for j in range(1, len(pts)):
            ax.plot(pts[j-1:j+1,0], pts[j-1:j+1,1], color=cmap(i), alpha=fade**(len(pts)-j), linewidth=3)
        ax.scatter(pts[-1,0], pts[-1,1], s=120, color=cmap(i), edgecolor='k', zorder=10)
        pred = clf.predict([pts[-1]])
        ax.text(pts[-1,0], pts[-1,1], str(pred[0]), fontsize=13, fontweight='bold', color='black', ha='center', va='center')
    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)
    ax.set_title(
        f"Navier-Stokes Meets Diffusion Models & Classification\n"
        f"Step {frame}: Probability Flow, Drift, Decision Boundaries",
...
- **Colored arrows** show the drift/score field, analogous to velocity in Navier-Stokes.
